...
static void		before_game_init(t_ptr *p)
{
	...
	p->p->y, "cub3D");													// man /Users/nastya/Desktop/man/man1/mlx_new_window.1
	p->img[0]->img_ptr = mlx_new_image(p->m->mlx_ptr,
	p->p->x, p->p->y);													// mlx_new_image.1
	p->img[0]->img_data = mlx_get_data_addr(p->img[0]->img_ptr,
	&p->img[0]->bpp, &p->img[0]->size_line, &p->img[0]->endian);		// mlx_new_image.1
}
...
static void		start_render(t_ptr *ptr, int fd)
{
	...
	ptr->p->y);															// mlx_new_image.1
	ptr->img[0]->img_data = mlx_get_data_addr(ptr->img[0]->img_ptr,
	&ptr->img[0]->bpp, &ptr->img[0]->size_line, &ptr->img[0]->endian);	// mlx_new_image.1
	run_dda(ptr);
	mlx_put_image_to_window(ptr->m->mlx_ptr, ptr->m->win_ptr,
	ptr->img[0]->img_ptr, 0, 0);										// mlx_new_image.1
}
