...
void			add_text(t_ptr *ptr, int flag, char *path)
{
	...
	path, &ptr->img[flag]->width, &ptr->img[flag]->height)))					// man /Users/nastya/Desktop/man/man1/mlx_new_image.1
		error("Texture path isn't valid");
	ptr->img[flag]->img_data = mlx_get_data_addr(ptr->img[flag]->img_ptr,
	&ptr->img[flag]->bpp, &ptr->img[flag]->size_line, &ptr->img[flag]->endian);	// mlx_new_image.1
}
...
void			init_game_struct(t_ptr *ptr)
{
	...
	ptr->m->mlx_ptr = mlx_init();												// mlx.1 CONNECTION INITIALIZATION
